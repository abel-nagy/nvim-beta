...
-- Define color segments for each line (shortened but same structure as your original)
local header_hl = {
  { { "AlphaHeader1", 7, 22 }, { "AlphaHeader2", 33, 50 } },
  { { "AlphaHeader3", 6, 21 }, { "AlphaHeader4", 33, 45 } },
  { { "AlphaHeader5", 6, 19 }, { "AlphaHeader6", 35, 45 }, { "AlphaHeader7", 45, 90 } },
  { { "AlphaHeader8", 4, 17 }, { "AlphaHeader9", 36, 90 } },
  { { "AlphaHeader10", 2, 17 }, { "AlphaHeader11", 38, 45 }, { "AlphaHeader12", 46, 90 } },
}

-- Your colors (you can tweak freely)
local colors = {
  "#c18250", "#5c441e", "#d6c383", "#407b48", "#98c09c",
  "#c38950", "#e0c785", "#44844b", "#a0c4a3", "#c7955b",
  "#e3cf88", "#4d9356", "#aecdb3",
}
for i, c in ipairs(colors) do
  vim.api.nvim_set_hl(0, "AlphaHeader" .. i, { fg = c })
end

local utils = require('alpha.utils')

-- Split logo lines & apply per-segment highlights
local header_val = vim.split(logo, "\n")
local bytehl = utils.charhl_to_bytehl(header_hl, header_val, false)
dashboard.section.header.val = header_val
dashboard.section.header.opts.hl = bytehl

---------- End of GPT Generated colored logo implementation
...
dashboard.section.buttons.val = {
    ...
    dashboard.button('p', '󱐥  Plugin view', '<cmd>lua require("plugin-view").open()<CR>'),
    dashboard.button('c', '󰈞  Find file in ~',
        ':cd ~ | silent Telescope find_files hidden=true no_ignore=true <CR>'),
    dashboard.button('d', '󰮗  Find file on HDD',
        ':cd /mnt/1tbData/ | silent Telescope find_files hidden=true no_ignore=true <CR>'),
    dashboard.button('s', '  Configure NeoVim', ':cd ' .. init_path .. '<CR>:e .<CR>'),
